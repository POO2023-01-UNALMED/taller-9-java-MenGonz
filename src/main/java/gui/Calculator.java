...
public class Calculator extends VBox implements EventHandler<ActionEvent>{
	...
	public Calculator(){
		...
		Rectangle rectangulo = new Rectangle(250, 50, Color.TRANSPARENT);
		
		
		rectangulo.setStroke(Color.GRAY);
		
		
		StackPane sp =  new StackPane(rectangulo, this.displayText);
	...
	public void handle(ActionEvent e) {
		...
		if(displayText.getText().contains("No")){
			...
			if(Integer.parseInt(valor)==0 && displayText.getText().equals("0")){
				displayText.setText("0");
			}else if(Integer.parseInt(valor)!=0 && displayText.getText().equals("0")){
				
				displayText.setText(valor);

			}else{
				displayText.setText(displayText.getText() + valor);
			}
		}else if(Arrays.asList(ops).contains(valor) && Arrays.stream(nums).anyMatch(num -> Arrays.asList(display_values).contains(num.toString())) && !Arrays.stream(ops).anyMatch(Arrays.asList(display_values)::contains)){

			displayText.setText(displayText.getText() + valor);

		}else if(Arrays.asList(ops).contains(valor) && Arrays.stream(ops).anyMatch(Arrays.asList(display_values)::contains) && !Arrays.asList(ops).contains(display_values[0]) && !esNumero(display_values[display.indexOf(Arrays.stream(ops).filter(display::contains).findFirst().orElse(null))+1])){
			
			displayText.setText(display.substring(0,display.length()-1) + valor);

		}else if(Arrays.asList(ops).contains(valor) && esNumero(display_values[display.indexOf(Arrays.stream(ops).filter(display::contains).findFirst().orElse(null))+1]) && !Arrays.asList(ops).contains(display_values[0])){
			
			displayText.setText(displayText.getText());
		
		}else if(Arrays.asList(ops).contains(valor) && Arrays.asList(ops).contains(display_values[0])){
